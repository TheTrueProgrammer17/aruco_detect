...
def process_visual(frame: np.ndarray,
    ...
    for m in markers:
        ...
        if not already:
            ...
            print(f"[VISUAL] ID={m.marker_id}  "
                  f"dist={m.distance_m:.2f}m  "
                  f"list={vs.markers_found}")
...
class GstreamNode(Node):
    def __init__(self):
        super().__init__('gstream_node')
        self.publisher = self.create_publisher(Image, 'camera/image_raw', 10)
        self.bridge = CvBridge()
        self.cap = cv2.VideoCapture(0)   # laptop webcam
        self.detector = setup_aruco()
    ...
    def loop(self):
        ...
        # Example: run docking mission processing
        vis = process_docking(frame, self.detector, self.state)
